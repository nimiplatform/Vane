...
if (process.argv[2] !== '--packaged') {
  ...
  const root = path.resolve(__dirname, '..', 'dist-electron-package',
    mac ? 'vane-shell-darwin-arm64/vane-shell.app/Contents' : 'vane-shell-win32-x64');
  const executable = path.join(root, mac ? 'MacOS/vane-shell' : 'vane-shell.exe');
  ...
  assert.equal(result.status, 0, `Packaged dependency check failed: ${result.signal || result.status}`);
} else {
  checkPackagedDependencies().catch((error) => { console.error(error); process.exitCode = 1; });
}
...
async function checkPackagedDependencies() {
  ...
  const fromApp = createRequire(path.join(resources, 'app.asar', 'package.json'));
  const manifest = fromApp('./package.json');
  assert.equal(manifest.version, require('../package.json').version);
  const fromKit = createRequire(fromApp.resolve('@nimiplatform/kit/shell/electron/main'));
  const native = process.platform === 'darwin'
    ? require(path.join(resources, 'nimi-native', 'protected-local', 'index.cjs'))
  ...
  const docx = zipSync({
    ...
    'word/document.xml': strToU8('<?xml version="1.0"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>'+phrase+'</w:t></w:r></w:p></w:body></w:document>'),
  });
  assert.ok((await officeParser.parseOffice(Buffer.from(docx))).toText().includes(phrase));
  ...
  try { assert.ok((await parser.getText()).text.includes(phrase)); } finally { await parser.destroy(); }
  // Vane routes PDF to PDFParse and DOCX to officeParser; preserve both in one Host.
  assert.ok((await officeParser.parseOffice(Buffer.from(docx))).toText().includes(phrase));
  
  console.log(`PASS: packaged Electron ${process.versions.electron} (${process.platform}/${process.arch}), protected native, canvas, DOCX/PDF/DOCX extraction`);
}
